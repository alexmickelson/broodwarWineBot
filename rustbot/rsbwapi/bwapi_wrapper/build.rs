...
fn main() {
  if env::var("CARGO_CFG_TARGET_OS").unwrap() != "windows" {
    panic!("RSBWAPI does not yet support targeting OpenBW, please compile with a Windows target.");
  }
  println!("cargo::rerun-if-changed=bwapi");

  // Check for both uppercase and lowercase BWAPI directory
  let bwapi_include = if PathBuf::from("bwapi/BWAPI/include").exists() {
    "bwapi/BWAPI/include"
  } else if PathBuf::from("bwapi/bwapi/include").exists() {
    "bwapi/bwapi/include"
  } else {
    panic!("BWAPI headers not found. Expected either bwapi/BWAPI/include or bwapi/bwapi/include");
  };
  ...
  let bindings = bindgen::Builder::default()
    .clang_arg("-xc++")
    .clang_arg("-std=c++14")
    .clang_arg(format!("-I{}", bwapi_include))
    .clang_arg("-I.")
