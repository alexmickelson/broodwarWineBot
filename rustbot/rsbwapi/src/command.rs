...
impl<'a> CommandApplier<'a> {
  ...
  fn add_string(&mut self, string: &str) -> usize {
    ...
    let len = bytes.len();
    let dst =
      unsafe { &mut *(&mut self.data.strings[string_count][..len] as *mut [i8] as *mut [u8]) };
...
impl Game {
  ...
  pub fn set_screen_position<P: Into<Position>>(&mut self, position: P) {
    let p = position.into();
    self.cmd().game_commands.push(BWAPIC_Command {
      type_: BWAPIC_CommandType_Enum::SetScreenPosition,
      value1: p.x,
      value2: p.y,
    });
  }
