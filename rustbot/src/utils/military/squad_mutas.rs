...
fn update_target_position(game: &Game, squad: &mut MilitarySquad) {
  ...
  // Check if leader has at least 5 units around it
  let leader_has_enough_units = check_leader_has_enough_units(game, squad, &squad_units);

  // Handle retreat if too few leading units or leader doesn't have enough units nearby
  if should_retreat(units_close_to_target, index, &path) || !leader_has_enough_units {
...
fn adjust_target_for_nearby_enemies(
  game: &Game,
...
fn get_leading_group_center(
  ...
  let leading_unit = if let Some(leader) = current_leader {
    ...
    let new_leader = squad_units.iter().min_by_key(|u| {
  ...
  if let Some(leader) = leading_unit {
    ...
    if !following_leader_units.is_empty() {
      let sum_x: i32 = following_leader_units
        .iter()
        .map(|u| u.get_position().x)
        .sum();
      let sum_y: i32 = following_leader_units
        .iter()
        .map(|u| u.get_position().y)
        .sum();
      (
        sum_x / following_leader_units.len() as i32,
        sum_y / following_leader_units.len() as i32,
      )
    } else {
...
fn handle_retreat_for_catching_up_units(
  ...
  let (group_center_x, group_center_y) =
    get_leading_group_center(game, squad, squad_units, current_target);
...
fn check_leader_has_enough_units(game: &Game, squad: &MilitarySquad, squad_units: &[Unit]) -> bool {
  ...
  let Some(leader) = game.get_unit(leader_id) else {
    return false;
  };

  if !leader.exists() || !squad.assigned_unit_ids.contains(&leader.get_id()) {
    return false;
  }
  ...
  let units_near_leader = squad_units
    .iter()
    .filter(|u| {
      if u.get_id() == leader_id {
        return false; // Don't count the leader itself
      }
      let pos = u.get_position();
      let dx = (pos.x - leader_pos.x) as f32;
      let dy = (pos.y - leader_pos.y) as f32;
  ...
  units_near_leader >= squad.required_units_near_leader
}
...
fn calculate_muta_defense_position(
  squad_units: &[Unit],
  ...
  let closest_group = muta_groups.iter().min_by_key(|group| {
...
fn check_enemies_near_buildings(game: &Game) -> Option<(i32, i32)> {
  ...
  let buildings: Vec<Unit> = game
    .get_all_units()
    .into_iter()
    .filter(|u| u.get_player().get_id() == player_id && u.get_type().is_building())
    .collect();
...
fn calculate_enemy_priority(enemy: &Unit, at_end_of_path: bool, squad_unit_ids: &std::collections::HashSet<usize>) -> Option<i32> {
  ...
  // Check if this enemy is attacking any squad member
  let is_attacking_squad = if let Some(target) = enemy.get_target() {
    squad_unit_ids.contains(&target.get_id())
  } else {
    false
  };

  // If attacking a squad member, highest priority
  if is_attacking_squad {
    return Some(-1);
  }
  ...
  let is_healer =
    unit_type == UnitType::Terran_Medic || unit_type == UnitType::Terran_Science_Vessel;
  ...
  // Priority: -1 = attacking squad (highest), 0 = anti-air units, 1 = workers, 2 = static defense and healers, 3 = other military units, 4 = buildings
  let priority = if has_air_weapon {
    0
  } else if is_worker {
    1
  } else if is_static_defense || is_healer {
    2
  } else if !is_building {
    3
  } else {
...
pub fn muta_unit_control(game: &Game, unit: &Unit, squad: &mut MilitarySquad) {
  ...
  if !nearby_enemies.is_empty() {
    ...
    let mut prioritized_enemies: Vec<(Unit, i32)> = nearby_enemies
      .into_iter()
      .filter_map(|enemy| {
        let priority = calculate_enemy_priority(&enemy, true, &squad.assigned_unit_ids)?;
        Some((enemy, priority))
      })
      .collect();
    ...
    if let Some((target_enemy, priority)) = prioritized_enemies.first() {
      if *priority == 4 {
        // If target is a building, attack move to its position instead
  ...
  handle_combat_at_target(game, unit, squad, (target_x, target_y), at_end_of_path);
}
...
fn handle_movement_to_target(
  ...
  if avoid_enemy_movement_utils::handle_threat_avoidance(
    ...
    unit,
    Some(target),
    ThreatAvoidanceMode::Kiting,
  ) {
    return;
  }

  // No threats, attack move to target
  attack_move_to_position(unit, target_pos);
}

fn handle_combat_at_target(game: &Game, unit: &Unit, squad: &MilitarySquad, target: (i32, i32), at_end_of_path: bool) {
  let unit_pos = unit.get_position();
  let target_pos = Position::new(target.0, target.1);

  // Get all nearby enemies
  let all_enemies = avoid_enemy_movement_utils::get_enemies_within(
    game,
    unit_pos,
    150.0,
    unit.get_player().get_id(),
  ...
  let mut prioritized_enemies: Vec<(Unit, i32)> = all_enemies
    .into_iter()
    .filter_map(|enemy| {
      let priority = calculate_enemy_priority(&enemy, at_end_of_path, &squad.assigned_unit_ids)?;
      Some((enemy, priority))
    })
    .collect();
  ...
  // No prioritized enemies, but check for any enemy to attack (closest)
  let all_nearby_enemies = avoid_enemy_movement_utils::get_all_enemies_within(
    game,
    unit_pos,
    300.0,
    unit.get_player().get_id(),
  ...
  if !all_nearby_enemies.is_empty() {
    // Filter out only eggs and larvae, keep everything else including buildings and workers
    let attackable_enemies: Vec<_> = all_nearby_enemies
      .iter()
      .filter(|enemy| {
        let unit_type = enemy.get_type();
        unit_type != UnitType::Zerg_Egg && unit_type != UnitType::Zerg_Larva
      })
      .collect();
    
    if !attackable_enemies.is_empty() {
      // Find closest enemy
      let closest_enemy = attackable_enemies
        .iter()
        .min_by_key(|enemy| {
          let enemy_pos = enemy.get_position();
          let dx = (enemy_pos.x - unit_pos.x) as f32;
          let dy = (enemy_pos.y - unit_pos.y) as f32;
          ((dx * dx + dy * dy).sqrt() * 100.0) as i32
        });
      
      if let Some(enemy) = closest_enemy {
        attack_unit_if_needed(unit, enemy);
        return;
      }
    }
  }

  // No enemies nearby at all, attack move to target position
  attack_move_to_position(unit, target_pos);
}
